...
`ParNew`收集器其实**跟`Paralle`收集器很类似**，区别主要在于它可以和`CMS`收集器配合使用。
...
2. **并发标记**：根据初始标记找到的根对象，使用深度优先遍历对象的成员变量进行标记
...
3. **再标记**：这个阶段需要暂停（STW），主要处理漏标对象，通过`SATB`算法解决（G1中的解决漏标的方案）。
...
>
> 目前ZGC历代版本中存在的一些问题（阿里、腾讯、美团、华为等大厂在支持业务切换 ZGC 的出现的），基本上都已经将遇到的相关问题和修复积极向社区报告和回馈，很多问题在JDK16和JDK17已经修复完善。另外的话，问题相对来说不是非常严重，如果遇到类似的问题可以查看下JVM团队的历代修复日志，建议尽量使用比较新的版本来上线，以免重复掉坑里面。
